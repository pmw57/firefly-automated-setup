...
export const InlineExpansionIcon: React.FC<InlineExpansionIconProps> = ({ type, className = "mx-1 align-bottom" }) => {
  ...
  if (meta.icon.type === 'sprite') {
    return (
      <span 
        className={`inline-block w-8 h-8 rounded shadow-sm border border-gray-300 bg-white shrink-0 ${className}`}
        style={{ 
    ...
    return (
      <span className={`inline-flex items-center justify-center w-8 h-8 rounded shadow-sm border border-gray-300 text-white font-bold text-xs shrink-0 ${bgColor} ${className}`} title={meta.label}>
        {meta.icon.value}
      </span>
    );
  }
