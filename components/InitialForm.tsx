...
import { SCENARIOS, EXPANSIONS_METADATA, SPRITE_SHEET_URL } from '../constants';
...
// Helper to get Expansion Icon using Metadata
...
export const ExpansionToggle: React.FC<ExpansionToggleProps> = ({ 
  ...
  return (
    <div 
      onClick={() => onToggle(id)}
      className={`
        relative cursor-pointer rounded-xl border-2 p-5 transition-all duration-300 ease-in-out flex items-center justify-between group
        ${active 
...
export const InitialForm: React.FC<InitialFormProps> = ({ gameState, setGameState, onStart }) => {
  ...
  const activeScenario = SCENARIOS.find(s => s.id === gameState.scenarioValue);

  // --- RENDER STEP 1: CAPTAINS & EXPANSIONS ---
  if (internalStep === 1) {
    return (
      <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 border border-gray-200 animate-fade-in">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center space-x-4 mb-4 select-none">
            <button 
              ...
              className="w-12 h-12 flex items-center justify-center rounded-lg bg-gray-100 hover:bg-gray-200 border border-gray-300 font-bold text-2xl text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              -
            </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-gray-50 p-4 rounded-lg border border-gray-100">
            {gameState.playerNames.map((name, index) => (
              <div key={index} className="flex items-center">
                <span className="text-xs font-bold text-gray-400 w-6 mr-1">{index + 1}.</span>
                <input
                  type="text"
                  value={name}
        ...
        <Button onClick={() => setInternalStep(2)} fullWidth className="text-lg py-3 shadow-md hover:translate-y-[-1px]">
  ...
  // --- RENDER STEP 2: SCENARIO SELECTION ---
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 border border-gray-200 animate-fade-in">
       <div className="flex justify-between items-center mb-6 border-b pb-2">
       ...
       <div className="mb-8 relative">
        ...
        {/* Full Visible List */}
        <div className="w-full border border-gray-200 rounded-lg shadow-sm bg-white overflow-hidden">
          {availableScenarios.map(opt => {
            ...
            return (
              <div 
                ...
                className={`flex items-stretch cursor-pointer border-b border-gray-100 last:border-0 transition-colors ${isSelected ? 'bg-green-50' : 'hover:bg-gray-50'}`}
              >
                 {/* Left Column: Icon */}
      ...
      <div className="flex gap-4">
        <Button onClick={() => setInternalStep(1)} variant="secondary" className="w-1/3">
          ← Back
        </Button>
