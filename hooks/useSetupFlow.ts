...
// Fix: Add missing type and constant imports
import { Step, GameState, SetupCardStep, SetupContentData, StepOverrides } from '../types';
...
// Fix: Move helper function logic into this file to resolve broken import from '../state/selectors'.
...
const getCoreStepsFromSetupCard = (state: GameState): Step[] => {
    ...
    if (isFlyingSolo) {
        ...
        if (!hasGameLengthStep) {
            const gameLengthStepDef = flyingSoloCard.steps.find(s => s.id === 'D_GAME_LENGTH_TOKENS');
...
const getOptionalRulesStep = (state: GameState): Step[] => {
    const isFlyingSolo = state.setupCardId === SETUP_CARD_IDS.FLYING_SOLO;
    if (state.expansions.tenth || isFlyingSolo) {
...
export const useSetupFlow = (): { flow: Step[] } => {
  ...
  const flow = useMemo(() => {
    ...
    return [
        ...
        ...getOptionalRulesStep(gameState),
        ...getCoreStepsFromSetupCard(gameState),
        getFinalStep(),
    ];
